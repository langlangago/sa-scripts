...
def match_idc(real_host, hname):
	...
	for key in real_host.keys():
		...
		for n in range(t3):
			name_url = 'http://www.domain.com/xxxx/getIdcByIp.do?ip=%s' % real_host[key][n]
...
def get_ips(vid):
	...
	for i in range(t):
		host_ip = []
		ips_url = 'http://www.domain.com/xxxx/cutVersinIntanceIP.do?versionId=%d ' % vid[i]
...
def get_vid(pname):
	...
	vid_url = 'http://www.domain.com/xxxx/listVersion.do?name=%s' % pname
...
def stop_pack(real_match):

	for x in real_match.keys():
		stop_url = 'http://www.domain.com/xxxx/stop.do?ips=%s&versionId=%s&key=xxx&retry=no&timeOut=240&executeType=0&interval=30&operator=dw_%s' % (",".join(real_match[x]), x,os.getlogin())
		stop_html = urllib2.urlopen(stop_url)
		stop_json = json.loads(stop_html.read())

		task_url = 'http://www.domain.com/xxxx/getDetailInfoByTaskId.do?task_id=%s' % stop_json['object']['taskId']
		task_html = urllib2.urlopen(task_url)
		task_json = json.loads(task_html.read())

		if stop_json['code'] == 0:
			print "package %s stop success!" % task_json['object'][0]['package_name']
		else:
			print "package %s stop error!" % task_json['object'][0]['package_name']
